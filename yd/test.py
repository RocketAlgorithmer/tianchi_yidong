...
def get_3data(df,time):
	time1=time_diff(time,7)
	time2=time_diff(time,14)
	time3=time_diff(time,21)
	one_week=split_by_time(df,time1,time)#最近一周
	two_week=split_by_time(df,time2,time)
	three_week=split_by_time(df,time3,time)
	return one_week,two_week,three_week
...
def user_weight_action_rate(df):
	df.reset_index(drop=True,inplace=True)
	...
	normalized_frame=min_max_normalize(grouped[frame.name].sum(),frame.name)#[frame.name]
	#normalized_frame.name='user_action_normalized_rate'
	return normalized_frame.rename(columns={frame.name:'user_weight_action_normalized_rate'})
...
def user_consume_want_rate(df):
	...
	user_consume_want=user_consume.join(frame)
	grouped=user_consume_want.groupby(user_label,as_index=False).sum()
	frame=pd.Series(list(map(lambda x,y: -1.0 if x==0 else float(x)/(float(y)+1),grouped['user_consume_counts'],grouped['user_want_counts'])))
...
def user_category_chongfudu(df):
	df.reset_index(drop=True,inplace=True)
	...
	user_chongfudu=user_chongfudu.groupby(user_label,as_index=False)[frame.name].sum().rename(columns={frame.name:'user_category_chongfudu'})
	return user_chongfudu


def user_category_want_chongfudu(df):
	df.reset_index(drop=True,inplace=True)
	frame =pd.Series(list(map(lambda x: 1. if x!=1 else 0. ,df[action_label])))
	frame.name = 'user_action_category'
	user_want_category = df[[user_label, category_label]].join(frame)
	...
	frame.name = 'user_action_category_rate'
	user_chongfudu=unique_user_category.join(frame)
	user_chongfudu=user_chongfudu.groupby(user_label,as_index=False)[frame.name].sum().rename(columns={frame.name:'user_category_want_chongfudu'})
	return user_chongfudu


def user_category_consume_chongfudu(df):
	df.reset_index(drop=True,inplace=True)
	frame =pd.Series(list(map(lambda x: 1. if x==4 else 0. ,df[action_label])))
	frame.name = 'user_action_category'
	user_want_category = df[[user_label, category_label]].join(frame)
	...
	frame.name = 'user_action_category_rate'
	user_chongfudu=unique_user_category.join(frame)
	user_chongfudu=user_chongfudu.groupby(user_label,as_index=False)[frame.name].sum().rename(columns={frame.name:'user_category_consume_chongfudu'})
	return user_chongfudu

category_chongfudu=[user_category_chongfudu,user_category_want_chongfudu,user_category_consume_chongfudu]

def user_item_chongfudu(df):
	df.reset_index(drop=True,inplace=True)
	frame =pd.Series(list(map(lambda x: 1.,df[action_label])))
	...
	user_chongfudu=user_chongfudu.groupby(user_label,as_index=False)[frame.name].sum().rename(columns={frame.name:'user_item_chongfudu'})
	return user_chongfudu

def user_item_want_chongfudu(df):
	df.reset_index(drop=True,inplace=True)
	frame =pd.Series(list(map(lambda x: 1. if x!=1 else 0.,df[action_label])))
	frame.name = 'user_want_item'
	user_want_item = df[[user_label, item_label]].join(frame)
	user_dicts = dict(user_want_item.groupby(user_label)[frame.name].sum())
	user_item_dicts = dict(user_want_item.groupby([user_label, item_label])[frame.name].sum())
	unique_user_item = df[[user_label, item_label]].drop_duplicates([user_label, item_label]).reset_index(drop=True)
	...
	frame = pd.Series(list(map(lambda x, y: (user_item_dicts[(x, y)]*user_item_dicts[(x, y)]) / ((user_dicts[x]+1)*(user_dicts[x]+1)), unique_user_item[user_label], unique_user_item[item_label])))
	frame.name = 'user_want_item_rate'
	user_chongfudu=unique_user_item.join(frame)
	user_chongfudu=user_chongfudu.groupby(user_label,as_index=False)[frame.name].sum().rename(columns={frame.name:'user__want_item_chongfudu'})
	return user_chongfudu


def user_item_consume_chongfudu(df):
	df.reset_index(drop=True,inplace=True)
	frame =pd.Series(list(map(lambda x: 1. if x!=1 else 0.,df[action_label])))
	...
	frame = pd.Series(list(map(lambda x, y: (user_item_dicts[(x, y)]*user_item_dicts[(x, y)]) / ((user_dicts[x]+1)*(user_dicts[x]+1)), unique_user_item[user_label], unique_user_item[item_label])))
	frame.name = 'user_consume_item_rate'
	user_chongfudu=unique_user_item.join(frame)
	user_chongfudu=user_chongfudu.groupby(user_label,as_index=False)[frame.name].sum().rename(columns={frame.name:'user__consume_item_chongfudu'})
	return user_chongfudu
...
def category_weight_action_rate(df):
	df.reset_index(drop=True,inplace=True)
	frame=pd.Series(list(map(action_weight,df[action_label])))
	frame.name='category_action_rate'
	action_category=df[[category_label]].join(frame)
	grouped=action_category.groupby(category_label,as_index=False)
	normalized_frame=min_max_normalize(grouped[frame.name].sum(),frame.name)#[frame.name]
	#normalized_frame.name='category_action_normalized_rate'
	return normalized_frame.rename(columns={frame.name:'category_weight_action_normalized_rate'})
...
def category_want_user_counts(df):
	...
	#print(grouped)
	return min_max_normalize(grouped, category_label).rename(columns={user_label: 'category_want_user_count_normalized'})
...
def category_consume_user_counts(df):
	...
	grouped = df[mask][[category_label, user_label]].groupby(category_label)[user_label].nunique().reset_index()
	#print(grouped)
	return min_max_normalize(grouped, category_label).rename(columns={user_label: 'category_consuem_user_count_normalized'})

category_user_counts=[category_action_user_counts,category_want_user_counts,category_consume_user_counts]


def category_consume_action_rate(df):
	df.reset_index(drop=True,inplace=True)
	frame = pd.Series(list(map(lambda x: 1. if x ==4 else 0., df[action_label])))
	frame.name='category_consume_counts'
	category_consume=df[[category_label]].join(frame)
	frame=df[action_label].map(lambda x: 1.)
	frame.name='category_action_counts'
	category_consume_want=category_consume.join(frame)
	grouped=category_consume_want.groupby(category_label,as_index=False).sum()
	frame=pd.Series(list(map(lambda x,y: -1.0 if x==0 else float(x)/float(y),grouped['category_consume_counts'],grouped['category_action_counts'])))
	frame.name='category_consume_action_rate'
	return grouped[[category_label]].join(frame)


def category_consume_want_rate(df):
	df.reset_index(drop=True,inplace=True)
	frame = pd.Series(list(map(lambda x: 1. if x ==4 else 0., df[action_label])))
	frame.name='category_consume_counts'
	category_consume=df[[category_label]].join(frame)
	frame=df[action_label].map(lambda x: 1. if x!=1 else 0.)
	frame.name='category_want_counts'
	category_consume_want=category_consume.join(frame)
	grouped=category_consume_want.groupby(category_label,as_index=False).sum()
	frame=pd.Series(list(map(lambda x,y: -1.0 if x==0 else float(x)/(float(y)+1),grouped['category_consume_counts'],grouped['category_want_counts'])))
	frame.name='category_consume_want_rate'
	return grouped[[category_label]].join(frame)
...
#物品
def item_want_rate(df):
	df.reset_index(drop=True,inplace=True)
	frame=pd.Series(list(map(lambda x: 1. if x!=1 else 0,df[action_label])))#1. if x!=1 else 0
	frame.name='item_want'
	item_want=df[[item_label]].join(frame)
	grouped=item_want.groupby(item_label,as_index=False)
	normalized_frame=min_max_normalize(grouped[frame.name].sum(),frame.name)[frame.name]
	normalized_frame.name='item_want_normalized_rate'
	return grouped[frame.name].mean().join(normalized_frame)

def item_action4_rate(df):
	df.reset_index(drop=True,inplace=True)
	frame=pd.Series(list(map(lambda x: 1. if x==4 else 0.,df[action_label])))
	frame.name='item_action4_rate'
	action_item=df[[item_label]].join(frame)
	grouped=action_item.groupby(item_label,as_index=False)
	normalized_frame=min_max_normalize(grouped[frame.name].sum(),frame.name)[frame.name]
	normalized_frame.name='item_action4_normalized_rate'
	return grouped[frame.name].mean().join(normalized_frame)

def item_weight_action_rate(df):
	df.reset_index(drop=True,inplace=True)
	frame=pd.Series(list(map(action_weight,df[action_label])))
	frame.name='item_action_rate'
	action_item=df[[item_label]].join(frame)
	grouped=action_item.groupby(item_label,as_index=False)
	normalized_frame=min_max_normalize(grouped[frame.name].sum(),frame.name)#[frame.name]
	#normalized_frame.name='category_action_normalized_rate'
	return normalized_frame.rename(columns={frame.name:'item_weight_action_normalized_rate'})

item_action_rates=[item_want_rate,item_action4_rate,item_weight_action_rate]

def item_action_user_counts(df):
	df.reset_index(drop=True,inplace=True)
	# 类目用户量
 #	 mask = pd.Series(list(map(lambda x: True if x!=1 else False, df[action_label])))
	#统计不同商家的数目并变成正常索引
	...
	#print(grouped)
	return min_max_normalize(grouped, user_label).rename(columns={user_label: 'item_action_user_count_normalized'})


def item_want_user_counts(df):
	df.reset_index(drop=True,inplace=True)
	# 用户想要的不同类目数量归一化
	mask = pd.Series(list(map(lambda x: True if x!=1 else False, df[action_label])))
	#统计不同类目的数目并变成正常索引
	...
	#print(grouped)
	return min_max_normalize(grouped, user_label).rename(columns={user_label: 'item_want_user_count_normalized'})


def item_consume_user_counts(df):
	df.reset_index(drop=True,inplace=True)
	# 用户想要的不同类目数量归一化
	mask = pd.Series(list(map(lambda x: True if x!=1 else False, df[action_label])))
	#统计不同类目的数目并变成正常索引
	grouped = df[mask][[item_label, user_label]].groupby(item_label)[user_label].nunique().reset_index()
	#print(grouped)
	return min_max_normalize(grouped, user_label).rename(columns={user_label: 'item_consume_user_count_normalized'})

item_user_counts=[item_action_user_counts,item_want_user_counts,item_consume_user_counts]


def item_consume_action_rate(df):
	df.reset_index(drop=True,inplace=True)
	frame = pd.Series(list(map(lambda x: 1. if x ==4 else 0., df[action_label])))
	frame.name='item_consume_counts'
	item_consume=df[[item_label]].join(frame)
	frame=df[action_label].map(lambda x: 1.)
	frame.name='item_action_counts'
	item_consume_want=item_consume.join(frame)
	grouped=item_consume_want.groupby(item_label,as_index=False).sum()
	frame=pd.Series(list(map(lambda x,y: -1.0 if x==0 else float(x)/float(y),grouped['item_consume_counts'],grouped['item_action_counts'])))
	frame.name='item_consume_action_rate'
	return grouped[[item_label]].join(frame)

def item_consume_want_rate(df):
	df.reset_index(drop=True,inplace=True)
	frame = pd.Series(list(map(lambda x: 1. if x ==4 else 0., df[action_label])))
	frame.name='item_consume_counts'
	item_consume=df[[item_label]].join(frame)
	frame=df[action_label].map(lambda x: 1. if x!=1 else 0.)
	frame.name='item_want_counts'
	item_consume_want=item_consume.join(frame)
	grouped=item_consume_want.groupby(item_label,as_index=False).sum()
	frame=pd.Series(list(map(lambda x,y: -1.0 if x==0 else float(x)/(float(y)+1),grouped['item_consume_counts'],grouped['item_want_counts'])))
	frame.name='item_consume_want_rate'
	return grouped[[item_label]].join(frame)
...
def add_item_features(df):
	...
	for f in item_features:
		item_feature_data=item_feature_data.merge(f(df),on=item_label,how='left')
...
def user_consume_category_rate(df):
	df.reset_index(drop=True,inplace=True)
	# 用户对每个类目的消费次数占用户普通关注所有类目的比重# 字典索引方式可能有问题
	frame = pd.Series(list(map(lambda x: 1. if x == 4 else 0., df[action_label])))
	frame.name = 'user_consume_category'
	user_want_category = df[[user_label, category_label]].join(frame)
	user_dicts = dict(user_want_category.groupby(user_label)[frame.name].sum())
	user_category_dicts = dict(user_want_category.groupby([user_label, category_label])[frame.name].sum())
	unique_user_category = df[[user_label, category_label]].drop_duplicates([user_label, category_label]).reset_index(drop=True)
	frame = pd.Series(list(map(lambda x, y: user_category_dicts[(x, y)] / (user_dicts[x]+1), unique_user_category[user_label], unique_user_category[category_label])))
	frame.name = 'user_consume_category_rate'
	return unique_user_category.join(frame)
...
def add_user_item_features(df):
	...
	for f in user_item_features:
		user_item_feature_data=user_item_feature_data.merge(f(df),on=[user_label,item_label],how='left')
	user_item_feature_data.fillna(-1,inplace=True)
	return user_item_feature_data

#类目和物品
def category_item_action_rate(df):
	df.reset_index(drop=True,inplace=True)
	# 每个物品在类别中受关注度
	frame = pd.Series(list(map(lambda x: 1. , df[action_label])))
	frame.name = 'item_category_action'
	...
	frame.name = 'category_item_action_rate'
...
#整个时间的特征先只用最近一周的特征
	...
	df=add_user_features(data1)
	user_features=user_features.merge(df,on=user_label,how='left')
	...
	df=add_category_features(data1)
	category_features=category_features.merge(df,on=category_label,how='left')
...
#合并后的特征
	...
	item_features=add_item_features(raw_data)
	df=add_item_features(data1)
	item_features=item_features.merge(df,on=item_label,how='left')
	...
	df=add_user_category_features(data1)
	user_category_features=user_category_features.merge(df,on=[user_label,category_label],how='left')
	...
	user_item_features=add_user_item_features(raw_data)
	df=add_user_item_features(data1)
	user_item_features=user_item_features.merge(df,on=[user_label,item_label],how='left')
	...
	df=add_category_item_features(data1)
	category_item_features=category_item_features.merge(df,on=[category_label,item_label],how='left')
